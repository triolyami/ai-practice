...
def breakdown(system_prompt: str, longterm: str, working: str, history: list[dict], request: str) -> dict:
    system = estimate_text(system_prompt or "")
    lt = estimate_text(longterm or "")
    lt_overhead = MESSAGE_OVERHEAD if lt else 0
    wk = estimate_text(working or "")
    wk_overhead = MESSAGE_OVERHEAD if wk else 0
    hist = sum(estimate_text(m.get("content", "")) + MESSAGE_OVERHEAD for m in history)
    req = estimate_text(request or "") + MESSAGE_OVERHEAD
    total = system + MESSAGE_OVERHEAD + lt + lt_overhead + wk + wk_overhead + hist + req
    return {
        "system": system,
        "longterm": lt,
        "working": wk,
