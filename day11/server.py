...
from agent import MAX_CONTENT, LAYER_KEYS, WORKSPACE_MAX_NAME, Agent
...
DIST_DIR = DAY11_DIR / "frontend" / "dist"
ASSET_TYPES = {
...
WORKSPACE_RE = re.compile(r"^[\w-]{1,80}$", re.UNICODE)
...
DB_PATH = DAY11_DIR / "data" / "chat_history.db"
...
def resolve_agent(sid: str) -> Agent | None:
    ...
    print(
        ...
        f"область {agent.workspace_id}, реплик {agent.describe()['turns']}",
        flush=True,
    )
    return agent
...
def parse_config(raw) -> tuple[dict | None, str | None]:
    ...
    if workspace is not None:
        if not isinstance(workspace, str):
            return None, "config.workspace должен быть строкой."
        if len(workspace) > WORKSPACE_MAX_NAME:
            return None, f"config.workspace длиннее {WORKSPACE_MAX_NAME} символов."
        out["workspace"] = workspace
    layers = raw.get("layers")
    if layers is not None:
        if not isinstance(layers, dict):
            return None, "config.layers должен быть объектом {short, working, longterm}."
...
class Day11Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            self._json(200, {"workspaces": STORE.list_workspaces()})
        else:
    ...
    def do_POST(self):
        ...
        handler = {
            ...
            "/api/promote": self._api_promote,
            "/api/workspace/delete": self._api_workspace_delete,
        }.get(path)
        if handler is None:
            self._json(404, {"error": "Нет такого адреса"})
            return
    ...
    def _dist_index(self):
        ...
        if not index.exists():
            body = "Фронтенд не собран. Выполните: cd day11/frontend && npm install && npm run build"
        ...
        try:
            if agent is None:
                ...
                if "workspace" in config:
                    agent.configure(workspace=config["workspace"])
                REGISTRY.put(sid, agent)
            else:
    ...
    def _stream_chat(self, sid: str, agent: Agent, message: str) -> None:
        ...
        print(
            ...
            f"область «{info['workspace'] or 'личная'}», слои {agent.layers}, "
                ...
                for event in gen:
                    ...
                    if event["event"] == "done":
                        ...
                        print(
                            ...
                            f"фактов в задаче {len(event['working']['facts'])}, "
    ...
    def _api_forget(self, body: dict) -> None:
        ...
        def drop():
            ...
            try:
                ...
                self._json(500, {"error": f"Не удалось удалить диалог из базы: {str(exc)[:200]}"})
                return
            wsid = (snapshot or {}).get("workspace_id") or sid
            with MEMORY_LOCK:
                try:
                    STORE.drop_covered(wsid, sid)
                except Exception:
                    pass
    ...
    def _working_put(self, body: dict) -> None:
        sid = self._session_id(body)
        if sid is None:
            return
        agent = resolve_agent(sid)
        if agent is None:
            self._json(404, {"error": "У этой сессии ещё нет агента — нечего редактировать."})
            return
        patch = {}
        if "goal" in body:
            if not isinstance(body["goal"], str):
                self._json(400, {"error": "goal должен быть строкой."})
                return
            patch["goal"] = body["goal"]
        if "plan" in body:
            if not isinstance(body["plan"], list) or not all(isinstance(p, str) for p in body["plan"]):
                self._json(400, {"error": "plan должен быть массивом строк."})
                return
            patch["plan"] = body["plan"]
        if "facts" in body:
            facts = body["facts"]
            ok = isinstance(facts, list) and all(
                isinstance(p, (list, tuple)) and len(p) >= 2
                and isinstance(p[0], str) and isinstance(p[1], str)
                for p in facts
            )
            if not ok:
                self._json(400, {"error": "facts должен быть массивом пар [ключ, значение]."})
                return
            patch["facts"] = [[p[0], p[1]] for p in facts]
        if not patch:
            self._json(400, {"error": "Передайте хотя бы одно поле: goal, plan, facts."})
            return
        with MEMORY_LOCK:
            row = STORE.load_workspace(agent.workspace_id) or {"name": "", "state": None, "covered": {}}
            state = normalize_working(row.get("state"))
            for key in ("goal", "plan", "facts"):
                if key in patch:
                    state[key] = patch[key]
            state = normalize_working(state)
            covered = row.get("covered") or {}
            STORE.save_workspace(agent.workspace_id, row.get("name") or "", state, covered)
        print(f"   рабочая память области {agent.workspace_id} отредактирована вручную", flush=True)
        self._json(200, {
            "ok": True,
            ...
            "working": {
                **state,
                "workspace": row.get("name") or "",
                "covered": covered.get(sid, 0) if isinstance(covered, dict) else 0,
            },
        })

    def _api_promote(self, body: dict) -> None:
        sid = self._session_id(body)
        if sid is None:
            return
        key = body.get("key")
        if not isinstance(key, str) or not key.strip():
            self._json(400, {"error": "key — непустая строка: ключ факта из рабочей памяти."})
            return
        agent = resolve_agent(sid)
        if agent is None:
            self._json(404, {"error": "У этой сессии ещё нет агента."})
            return
        if not agent.promote(key.strip()):
            self._json(404, {"error": f"Факт «{key.strip()}» не найден в рабочей памяти."})
            return
        row = STORE.load_workspace(agent.workspace_id) or {}
        state = normalize_working(row.get("state"))
        covered = row.get("covered") or {}
        print(f"   факт «{key.strip()}» перенесён в долговременную память", flush=True)
        self._json(200, {
            "ok": True,
            "workspace": row.get("name") or "",
            "working": {
    ...
    def _api_workspace_delete(self, body: dict) -> None:
        wsid = body.get("workspace_id")
        if not isinstance(wsid, str) or not WORKSPACE_RE.match(wsid):
            self._json(400, {"error": "workspace_id — строка из букв, цифр, - и _ (до 80 символов)."})
            return
        with MEMORY_LOCK:
            sids = STORE.delete_workspace(wsid)
            for sid in sids:
                agent = REGISTRY.get(sid)
                if agent is not None:
                    agent.workspace_id = sid
        print(f"   область {wsid} удалена, чатов вернулось к личной памяти: {len(sids)}", flush=True)
        self._json(200, {"ok": True, "reverted": sids})


def main():
    server = ThreadingHTTPServer(("0.0.0.0", PORT), Day11Handler)
    print(
        f"День 11, слои памяти (краткосрочная / рабочая / долговременная): "
