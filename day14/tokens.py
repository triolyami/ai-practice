...
def breakdown(system_prompt: str, invariants: str, history: list[dict], request: str) -> dict:
    system = estimate_text(system_prompt or "")
    inv = estimate_text(invariants or "")
    inv_overhead = MESSAGE_OVERHEAD if inv else 0
    hist = sum(estimate_text(m.get("content", "")) + MESSAGE_OVERHEAD for m in history)
    req = estimate_text(request or "") + MESSAGE_OVERHEAD
    total = system + MESSAGE_OVERHEAD + inv + inv_overhead + hist + req
    return {
        "system": system,
        "invariants": inv,
        "history": hist,
