import { ENFORCE_LABEL, LAYERS, fmtTokens } from '../lib/constants.js'
import { plural } from '../lib/format.js'

export const SEGMENTS = [
  { key: 'system', label: 'система', color: '#787774' },
  { key: 'invariants', label: 'инварианты', color: '#5B4A9F' },
      ...
      {preview && (
        <div className="ctx-next" title="оценка состава запроса, который уйдёт при следующем ходе">
          <div className="ctx-next-head">
            <span>
              следующий запрос
              {preview.invariant_name ? ` · ${preview.invariant_name}` : ''}
              {preview.invariant_missing ? ' · набор не найден' : ''}
            </span>
          ...
          <span className="ctx-next-sub">
            ...
            {preview.enforce ? ` · инварианты: ${ENFORCE_LABEL[preview.enforce] || preview.enforce}` : ''}
