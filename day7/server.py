...
DAY7_DIR = Path(__file__).parent
DIST_DIR = DAY7_DIR / "frontend" / "dist"
ASSET_TYPES = {
...
DB_PATH = DAY7_DIR / "data" / "chat_history.db"
...
def resolve_agent(sid: str) -> Agent | None:
    ...
    print(
        f"   восстановлен диалог сессии {sid} из базы: «{agent.name}», реплик {agent.describe()['turns']}",
        flush=True,
    )
    return agent
...
class Day7Handler(BaseHTTPRequestHandler):
    ...
    def _dist_index(self):
        ...
        if not index.exists():
            body = "Фронтенд не собран. Выполните: cd day7/frontend && npm install && npm run build"
...
def main():
    server = ThreadingHTTPServer(("0.0.0.0", PORT), Day7Handler)
    print(f"День 7, сохранение контекста: http://127.0.0.1:{PORT} (или eth0-IP из WSL), Ctrl+C — остановка", flush=True)
