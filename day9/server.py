...
from agent import COMPRESSION_MODES, MAX_CONTENT, Agent
from config import MODELS, thinking_label
from storage import ChatStore

DAY9_DIR = Path(__file__).parent
DIST_DIR = DAY9_DIR / "frontend" / "dist"
ASSET_TYPES = {
...
DB_PATH = DAY9_DIR / "data" / "chat_history.db"
...
def resolve_agent(sid: str) -> Agent | None:
    ...
    print(
        ...
        f"режим {agent.compression}, реплик {agent.describe()['turns']}",
        flush=True,
    )
    return agent
...
def parse_config(raw) -> tuple[dict | None, str | None]:
    ...
    if compression is not None:
        if compression not in COMPRESSION_MODES:
            return None, (
                f"config.compression должен быть одним из: {', '.join(COMPRESSION_MODES)}."
            )
        out["compression"] = compression
    return out, None


class Day9Handler(BaseHTTPRequestHandler):
    ...
    def _dist_index(self):
        ...
        if not index.exists():
            body = "Фронтенд не собран. Выполните: cd day9/frontend && npm install && npm run build"
    ...
    def _stream_chat(self, sid: str, agent: Agent | None, config: dict, message: str) -> None:
        ...
        print(
            ...
            f"режим {agent.compression}, реплик в памяти {agent.describe()['turns']}",
            flush=True,
        )
        done_event = None
        try:
            ...
            try:
                for event in gen:
                    ...
                    if event["event"] == "done":
                        ...
                        print(
                            f"   история сжата: −{event['compressed_messages']} сообщ., "
                            f"покрыто сводкой {event['covered']}, "
                            f"символов в сводке {event['summary_chars']}",
                            flush=True,
                        )
            except ClientGone:
...
def main():
    server = ThreadingHTTPServer(("0.0.0.0", PORT), Day9Handler)
    print(f"День 9, сжатие истории: http://127.0.0.1:{PORT} (или eth0-IP из WSL), Ctrl+C — остановка", flush=True)
