import { COMPRESSION_MODES, MODELS, fmtMoney, fmtTokens } from '../lib/constants.js'
...
export default function TokenPanel({ metas, model, totals }) {
  ...
  const summaryCalls = byMode?.summary_calls
    ...
    <section className="tokenpanel">
      <div className="tokenpanel-in">
        ...
        <div className="tp-block tp-compare">
          <span className="mini-label">расход по режимам (промпт + ответ · стоимость)</span>
          {byMode ? (
            <table className="tp-table">
              <tbody>
                {Object.entries(COMPRESSION_MODES).map(([mode, label]) => {
                  ...
                  return (
                    <tr key={mode} className={last?.mode === mode ? 'tp-row--on' : ''}>
                      <td>{label}</td>
                ...
                {summaryCalls && (
                  <tr className="tp-row--summary" title="отдельные вызовы модели на сжатие истории">
                    <td>вызовы summary</td>
                    <td>{summaryCalls.requests} · {fmtTokens(summaryCalls.prompt_tokens)} + {fmtTokens(summaryCalls.completion_tokens)}</td>
                    <td>{fmtMoney(summaryCalls.cost_usd)}</td>
                  </tr>
                )}
              </tbody>
            </table>
