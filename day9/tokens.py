...
def breakdown(system_prompt: str, summary: str, history: list[dict], request: str) -> dict:
    system = estimate_text(system_prompt or "")
    summ = estimate_text(summary or "")
    summ_overhead = MESSAGE_OVERHEAD if summ else 0
    hist = sum(estimate_text(m.get("content", "")) + MESSAGE_OVERHEAD for m in history)
    req = estimate_text(request or "") + MESSAGE_OVERHEAD
    total = system + MESSAGE_OVERHEAD + summ + summ_overhead + hist + req
    return {"system": system, "summary": summ, "history": hist, "request": req, "total": total}
