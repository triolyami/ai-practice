import { MODELS, MODEL_NOTES, OVERFLOW_MODES } from '../lib/constants.js'
...
export default function SettingsPanel({ settings, setSettings, onReset, canReset, busy, turns }) {
  ...
  return (
    <div className="settings">
      ...
      <div className="set-row">
        <div className="set-head">
          <span className="set-name">При переполнении контекста</span>
          <div className="seg seg--sm" role="group">
            {Object.entries(OVERFLOW_MODES).map(([mode, label]) => (
              <button
                key={mode}
                type="button"
                className={settings.overflow === mode ? 'active' : ''}
                onClick={() => set('overflow', mode)}
              >
                {label}
              </button>
            ))}
        ...
        <p className="set-note">
          «ошибка» — запрос уходит модели как есть; если она отклонит его из-за
          переполнения контекста, вы увидите ошибку провайдера;
          «обрезать историю» — при таком отказе агент выбросит самые старые реплики
          и повторит запрос, пока модель не примет (потеря памяти будет видна в ответах)
        </p>
      </div>
