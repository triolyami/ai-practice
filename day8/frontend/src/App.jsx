...
export default function App() {
  ...
  const resetMemory = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        setMessages([])
        setNotice('Память агента очищена: личность и настройки сохранены, диалог и счётчики токенов обнулены.')
      } else {
  ...
  const contextWarning =
    lastPct >= 80
      ? `Контекст заполнен на ${lastPct}% — следующий запрос может не влезть. Включите «обрезать историю» в настройках или начните новый чат.`
      : null
