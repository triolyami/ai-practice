...
DAY8_DIR = Path(__file__).parent
DIST_DIR = DAY8_DIR / "frontend" / "dist"
ASSET_TYPES = {
...
def parse_config(raw) -> tuple[dict | None, str | None]:
    ...
    if overflow is not None:
        if overflow not in ("error", "trim"):
            return None, "config.overflow должен быть «error» или «trim»."
        out["overflow"] = overflow
    return out, None


class Day8Handler(BaseHTTPRequestHandler):
    ...
    def _stream_chat(self, sid: str, agent: Agent | None, config: dict, message: str) -> None:
        ...
        print(
            ...
            f"режим переполнения {agent.overflow}, реплик в памяти {agent.describe()['turns']}",
            flush=True,
        )
        finished = None
        try:
            ...
            if finished is not None and finished.get("event") == "done":
                ...
                print(
                    f"   ответ готов: промпт {meta['prompt_tokens']} + ответ {meta['completion_tokens']} "
                    f"токенов{cost_note}, {meta['latency_ms']} мс",
                    flush=True,
                )
...
def main():
    server = ThreadingHTTPServer(("0.0.0.0", PORT), Day8Handler)
    print(f"День 8, токены: http://127.0.0.1:{PORT} (или eth0-IP из WSL), Ctrl+C — остановка", flush=True)
    try:
