...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    strategy TEXT NOT NULL DEFAULT 'window',
...
class ChatStore:
    ...
    def save(self, session_id: str, snapshot: dict) -> None:
        ...
        facts_state = snapshot.get("facts_state")
        ...
        with closing(self._connect()) as conn, conn:
            conn.execute(
                ...
                "  strategy, window_size, facts_state, parent_id, fork_len, stats, updated_at)"
                " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    session_id,
                    snapshot["name"],
                    ...
                    snapshot.get("strategy", "window"),
                    snapshot.get("window_size", 10),
                    json.dumps(facts_state, ensure_ascii=False) if facts_state is not None else None,
                    snapshot.get("parent_id"),
                    snapshot.get("fork_len"),
                    stats,
                    time.time(),
                ),
            )
    ...
    def load(self, session_id: str) -> dict | None:
        with closing(self._connect()) as conn:
            row = conn.execute(
                ...
                " strategy, window_size, facts_state, parent_id, fork_len, stats"
