export const COUNTER_BUILD = '2026-09-14.5'
...
export function snapshot(settings) {
  const size = Number(settings.windowSize)
  return {
    name: settings.name.trim(),
    system_prompt: settings.systemPrompt.trim(),
    model: settings.model,
    strategy: settings.strategy in STRATEGY_MODES ? settings.strategy : 'window',
    window_size: Number.isFinite(size) ? Math.round(size) : DEFAULT_AGENT.windowSize,
  }
}
